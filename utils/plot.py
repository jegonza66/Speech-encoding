...
def define_ticks(
    ...
    if ylabel=='Bin-Envelope':    
        tags = np.load(config.saves_dir+r'\preprocessed_data\tmin-0.2_tmax0.6\stimuli_labels\bin_edges.npy')
        tags = (tags+(tags[-1]-tags[-2])//2)[:-1]
        ticks = np.arange(number_of_ticks)
    ...
    if ylabel.startswith('Spectrogram'):
        ...
        tags = tags[::2]
    elif ylabel=='Bin-Envelope':  
        ylabel = 'Envelope Bin Centers (dB)'
...
def channel_weights(
    ...
    fig.suptitle(f'Session {session} - Subject {subject} - Mcorr: {best_correlation:.2f} - '+ r'$\alpha$'+f': {best_alpha:.2f}')
...
def hyperparameter_selection(
    ...
    """
    Combined hyperparameter selection plot with correlation and TRFs visualization.
    (RMSE removed)
    """
    plt.close()
    if no_figures:
