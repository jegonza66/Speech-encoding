...
w_diff = w_ridge-w_mtrf
w_rel_diff = 100*(np.abs(w_diff))/(np.abs(w_mtrf+w_ridge)/2)

# TODO: there are values that goes to inf and -inf. Son 5
try:
    number_of_infs = np.unique(w_rel_diff==-np.inf, return_counts=True)[1][1]
...
# ===================================================
# Make plot of weights relative porcentual difference
...
# Porcentual relative difference
evoked_diff = mne.EvokedArray(data=w_diff, info=info)
